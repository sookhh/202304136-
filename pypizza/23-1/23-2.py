...
# 이미지 로드 (png 형식으로 변경)
pizza_dough = pygame.image.load("pizza_dough1.png")
bacon_img = pygame.image.load("bacon.png")
cheese_img = pygame.image.load("cheese.png")
pepperoni_img = pygame.image.load("peperoni.png")
...
time_limit_per_game = 10
total_pizza_games = 10
current_pizza_game = 0
max_toppings_per_game = 10
...
while running:
    ...
    if not game_started:
        if topping_list:
            ...
            if topping_added:
                # 마지막에 총 얼마 벌었는지 표시
                total_earnings_text = font.render(f"총 수익: {max_pizza_price * total_pizza_games - current_pizza_price}원", True, (0, 0, 0))
                screen.blit(total_earnings_text, (screen_width // 2 - total_earnings_text.get_width() // 2, 300))
                pygame.display.flip()
                pygame.time.delay(2000)  # 2초 대기 (2000 밀리초)
        else:
            # 게임 시작 버튼
        ...
        # 토핑이 10개일 때 모두 이미지 표시
        if len(topping_list) == max_toppings_per_game:
            for i, topping in enumerate(topping_list):
                screen.blit(topping.image, (i * 50, 400))
